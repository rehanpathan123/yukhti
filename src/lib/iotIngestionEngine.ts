...
export function getSampleHardwareConfig(machineId: string, chcCode = 'CHC-MP-SEH-01') {
  const endpointUrl = `${process.env.VITE_SUPABASE_URL || 'https://vrmycvndfylrzuuxjrat.supabase.co'}/functions/v1/telemetry-webhook`;
  ...
  -H "Authorization: Bearer ${process.env.VITE_SUPABASE_ANON_KEY || 'your-supabase-anon-key'}" \\
