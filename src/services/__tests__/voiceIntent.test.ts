...
describe('Multilingual Voice AI Assistant & Intent Engine', () => {
  ...
  it('10. Blocks and flags off-topic queries (e.g. politics, coding) with standard warnings', () => {
    const offTopicQuery1 = 'who is the prime minister of india?';
    const offTopicQuery2 = 'write a python function to add two numbers';

    expect(intentService.isQueryOffTopic(offTopicQuery1)).toBe(true);
    expect(intentService.isQueryOffTopic(offTopicQuery2)).toBe(true);

    // Agriculture query is on-topic
    expect(intentService.isQueryOffTopic('wheat harvesting machine price')).toBe(false);

    // Test through full dialog turn process
    const result = conversationService.processUserTurn({
      sessionId: 'test-session-offtopic',
      userInputText: 'भारत का प्रधानमंत्री कौन है?',
      machines: SEEDED_MACHINES,
      context: rameshContext,
    });

    expect(result.success).toBe(false);
    expect(result.assistant_response_text).toContain('कृषि');
  });
});
