...
export class ConversationService {
  ...
  public processUserTurn(params: {
    ...
    // 1.1 Check if query is off-topic
    if (this.intentService.isQueryOffTopic(userInputText)) {
      const isHindi = userInputText.match(/[\u0900-\u097F]/) || session.conversation_history.some(h => h.text && h.text.match(/[\u0900-\u097F]/));
      const offTopicResponse = isHindi
        ? "क्षमा करें, मैं केवल खेती, फसल और कृषि-मशीनरी (जैसे हार्वेस्टर, ट्रैक्टर बुकिंग या मौसम) से जुड़े प्रश्नों के उत्तर दे सकता हूँ।"
        : "I'm sorry, I can only answer questions related to agriculture, crop cycles, and machinery rentals.";
      ...
      return {
        ...
        language_detected: isHindi ? 'hi' : 'en',
        intent_data: session.current_intent,
        missing_fields: session.missing_fields,
        matched_machines: [],
        assistant_response_text: offTopicResponse,
