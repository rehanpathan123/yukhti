...
export class IntentService {
  ...
  public isQueryOffTopic(text: string): boolean {
    const clean = text.trim().toLowerCase();
    
    // If it's a very short response (e.g. yes/no/numbers), allow it as it's likely answering a prompt.
    if (clean.length < 3) return false;
    
    // Explicit off-topic triggers
    const offTopicTriggers = [
      'prime minister', 'president', 'modi', 'rahul gandhi', 'capital of', 'rajdhani', 'currency', 'dollar',
      'python', 'javascript', 'html', 'css', 'write a code', 'programming', 'software', 'joke', 'chutkula',
      'song', 'sing', 'music', 'gaana', 'gaane', 'film', 'movie', 'actor', 'actress', 'cricket', 'dhoni',
      'kohli', 'football', 'joke', 'chutkule', 'कहानी', 'कहानियां', 'चुटकुला', 'गाने', 'गाना', 'फिल्म',
      'राजनीति', 'politics', 'pm of', 'president of', 'प्रधानमंत्री', 'राष्ट्रपति', 'मुख्यमंत्री', 'pm', 'cm'
    ];

    if (offTopicTriggers.some(trigger => clean.includes(trigger))) {
      return true;
    }
    ...
    const onTopicKeywords = [
      // Crops
      'गेहूं', 'gehu', 'wheat', 'सोयाबीन', 'soybean', 'धान', 'paddy', 'rice', 'कपास', 'cotton',
      'मक्का', 'maize', 'सरसों', 'mustard', 'चना', 'gram', 'फसल', 'crop', 'agriculture', 'खेती', 'kheti',
      // Tasks / Operations
      'कटाई', 'katai', 'harvest', 'काटना', 'जुताई', 'jotai', 'plough', 'plow', 'बुवाई', 'buwai', 'bovai', 'sowing',
      'छिड़काव', 'chhidkaav', 'spray', 'मड़ाई', 'madai', 'thresh', 'ढुलाई', 'transport', 'tractor', 'ट्रैक्टर',
      'harvester', 'हार्वेस्टर', 'rotavator', 'रोटावेटर', 'seeder', 'सीडर', 'sprayer', 'थ्रेशर', 'thresher',
      'trolley', 'ट्रॉली', 'trailer', 'मशीन', 'machine', 'उपकरण', 'equipment',
      // Common conversational/booking terms
      'बुक', 'book', 'किराया', 'rate', 'rent', 'price', 'पैसा', 'limit', 'credit', 'लोन', 'loan', 'agricredit',
      'मौसम', 'weather', 'rain', 'barish', 'बारिश', 'खेत', 'khet', 'village', 'गांव', 'जिला', 'district', 'sehore', 'सीहोर',
      'bilkisganj', 'बिल्किसगंज', 'हाँ', 'haan', 'yes', 'confirm', 'नहीं', 'nahi', 'no', 'cancel', 'रद्द',
      // Assistant identity
      'saathi', 'साथी', 'yukti', 'युक्ति', 'kisanops', 'कौन हो', 'kaun ho', 'who are you', 'help', 'मदद'
    ];

    // If query is long and contains absolutely no agricultural/machinery markers, flag it as off-topic.
    if (clean.length > 15) {
      const hasOnTopic = onTopicKeywords.some(keyword => {
        if (keyword.length <= 3) {
          // Prevent false substring matches on short words (e.g. 'धान' in 'प्रधानमंत्री') by matching whole words only
          const esc = keyword.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
          // Match boundary of spaces, punctuation, or start/end of string
          const regex = new RegExp(`(?:^|[\\s\\p{Punctuation}])${esc}(?:$|[\\s\\p{Punctuation}])`, 'u');
