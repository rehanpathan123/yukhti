...
function generateMockInvoicesForBookings(bookings: Booking[]): Invoice[] {
  return bookings.map((b, idx) => {
    const subtotal = b.bookedHours * b.hourlyRate;
    const transport = 300;
    const platform = 100;
    const discount = 100;
    const beforeTax = subtotal + transport + platform - discount;
    ...
    return {
      ...
      chcName: b.chcName,
      machineName: b.machineModel,
      machineIdentifier: b.machineIdentifier,
      rentalPeriod: `${new Date(b.startTime).toLocaleDateString()} - ${new Date(b.endTime).toLocaleDateString()}`,
      bookedHours: b.bookedHours,
      actualHours: b.bookedHours,
      baseRatePerHour: b.hourlyRate,
      baseRentalAmount: subtotal,
      ...
      paymentStatus: b.status === 'COMPLETED' ? 'CAPTURED' as const : 'PENDING' as const,
      issuedAt: new Date(b.startTime).toISOString(),
      items: []
    };
  });
}
...
export function getPopulatedDemoState(): AppState {
  const demoBookings = SEEDED_BOOKINGS;
  return {
    ...
    invoices: generateMockInvoicesForBookings(demoBookings),
    currentTelemetry: {},
    simulationState: getInitialSimulationState(),
    isSimulating: true,
    activeDemoScene: 1,
...
function notify() {
  try {
    storage.setItem(STORAGE_KEY, JSON.stringify(globalState));
  } catch (e) {
    ...
    loginUser: (profile: UserProfile) => {
      ...
      if (isDemoUser) {
        ...
        globalState = {
          ...
          invoices: generateMockInvoicesForBookings(SEEDED_BOOKINGS),
          maintenanceAlerts: SEEDED_MAINTENANCE_ALERTS,
          notifications: SEEDED_NOTIFICATIONS,
        };
      } else {
